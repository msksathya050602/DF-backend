...
export class BaseController implements BaseControllerAttributes {
    ...
    public badRequest(res: Response, message: string, options: Record<string, any> = {}) {
        this.logger.error(JSON.stringify({ error_code: STATUSCODES.BAD_REQUEST, error_message: message }));
        return res.status(STATUSCODES.BAD_REQUEST).json({ error_code: STATUSCODES.BAD_REQUEST, error_message: message, ...options });
    }

    public internalServerError(res: Response, message: string) {
        this.logger.error(JSON.stringify({ error_code: STATUSCODES.INTERNAL_SERVER_ERROR, error_message: message }));
    ...
    public notFound(res: Response, message: string, options: Record<string, any> = {}) {
        this.logger.error(JSON.stringify({ error_code: STATUSCODES.NOT_FOUND, error_message: message }));
        return res.status(STATUSCODES.NOT_FOUND).json({ error_code: STATUSCODES.NOT_FOUND, error_message: message, ...options });
    }

    public conflict(res: Response, message: string) {
        this.logger.error(JSON.stringify({ error_code: STATUSCODES.CONFLICT, error_message: message }));
    ...
    public forbidden(res: Response, message: string) {
        this.logger.error(JSON.stringify({ error_code: STATUSCODES.FORBIDDEN, error_message: message }));
    ...
    public unauthorized(res: Response, message: string) {
        this.logger.error(JSON.stringify({ error_code: STATUSCODES.UNAUTHORIZED, error_message: message }));
