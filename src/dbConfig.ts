...
function loadCaPem(): string | undefined {
    ...
    if (!fs.existsSync(caPath)) {
        throw new Error(`PG SSL CA file not found at ${caPath}. Mount the CA bundle and set PG_SSL_CA or CONFIG.database.ssl.caPath.`);
    }
...
const dbConfigOptions: PostgresConnectionOptions = {
    ...
    synchronize: false,
    ssl: ssl,
    logging: process.env.NODE_ENV === 'testing' ? true : ['error', 'migration', 'warn'],
